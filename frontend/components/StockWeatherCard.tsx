  ...
  return (
    <div className={clsx(
      ...
      <div className="flex justify-between items-start mb-4">
        ...
        <span className="text-3xl">{stock.weather_icon}</span>
      </div>

      {/* 종목 정보 */}
      <h3 className="font-bold text-lg mb-1 line-clamp-1">{stock.name}</h3>
      <p className="text-sm text-gray-600 mb-4">{stock.ticker} · {stock.sector}</p>
        ...
        <div className="w-full bg-gray-200 rounded-full h-2">
          <div
            className={clsx(
              ...
              stock.probability >= 0.7 ? 'bg-blue-500' :
              stock.probability >= 0.5 ? 'bg-green-500' :
              stock.probability >= 0.3 ? 'bg-yellow-500' : 'bg-red-500'
            )}
          ...
          <div className="flex items-center">
            {[1, 2, 3, 4, 5].map((star) => (
              <svg
                ...
                className={clsx(
                  'w-3 h-3',
                  star <= Math.round(stock.confidence * 5) ? 'text-yellow-400' : 'text-gray-300'
                )}
                fill="currentColor"
                viewBox="0 0 20 20"
              >
